<template>
  <div class="container my-2">
    ...
    <form @submit.prevent="onSubmit">
      <textarea v-model="questionBody" class="form-control" rows="4" placeholder="What do you want to ask?">
  ...
  methods: {
    async performNetworkRequest() {
      ...
      if(this.slug !== undefined && this.slug !== "") {
        endpoint = `/api/v1/questions/${this.slug}/`
        method = "PUT"
      }
      try {
    ...
    onSubmit() {
      if(!this.questionBody) {
        this.error = "Can't post an empty question"
      } else if (this.questionBody.length > 240) {
        this.error = "This field has no more than 240 characters"
      } else {
